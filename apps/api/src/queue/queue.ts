...
function Queue({ queueTimeout = 500, executionTimeout = 250, concurrency = 1, maxTaskCount = 1 } = {}): QueueInstance {
    ...
    function add<T>(fn: () => Promise<T>, onCancel?: () => void): CancellablePromise<T> {
        if (isFull()) {
...
function cancellable<T>(promise: Promise<T>, cancel: () => void): CancellablePromise<T> {
