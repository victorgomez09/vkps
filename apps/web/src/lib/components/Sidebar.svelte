...
<div class="shadow h-full w-48 hidden md:block bg-base-300">
					...
					>
						<div class="grid grid-cols-1 p-3">
							{#each themes as theme}
								<label
									class="outline-base-content text-start mb-2"
								...
								>
										...
										<span class="grid grid-cols-5 grid-rows-3">
											...
											>
												<svg
													...
													viewBox="0 0 24 24"
													fill="currentColor"
													class="hidden h-3 w-3 shrink-0"
													class:!block={theme === $selectedTheme}
												>
													<path
									...
									<input
										...
										aria-label={theme}
										value={theme}
										on:change={(event) => saveThemeStorage(event.currentTarget.value || 'default')}
									/>
								</label>
							{/each}
						</div>
