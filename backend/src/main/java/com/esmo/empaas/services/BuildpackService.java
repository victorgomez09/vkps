...
public class BuildpackService {
    ...
    public BuildpackEntity select(String projectDir) {
        ...
        if (fileUtil.searchFile(projectDir, "package.json")) {
            buildpack = buildpackRepository.findByName("nodejs").orElseThrow(() -> new ResourceNotFoundException("Buildpack", "name", "nodejs"));
        }
    ...
    public BuildpackEntity findByName(String name) {
        return buildpackRepository.findByName(name).orElseThrow(() -> new ResourceNotFoundException("Buildpack", "name", name));
    }
    ...
    public void build(String baseImage, String workdir, int port, String installCommand,
                      String buildCommand, String startCommand, String baseDirectory, Map<String, String> envs) {
        switch (baseImage.split(":")[0]) {
            case "nodejs":
                dockerUtil.createNodeJsDockerFile(baseImage, workdir, port, installCommand, buildCommand, startCommand, baseDirectory, envs);
